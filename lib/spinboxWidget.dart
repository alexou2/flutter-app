...
class WrapperModel extends ChangeNotifier {
  ...
  void addXp(Wrapper wrap, double xp) {
    // _items.add(product);
    wrap.Xp += xp;
    notifyListeners();
  }
...
class spinboxWidget extends StatelessWidget {
  spinboxWidget({Key? key, required this.contents}) : super(key: key);
    ...
    for (Wrapper ee in contents) {
      var spin = Row(
        children: [
          ...
          Column(
            children: [
              Consumer<WrapperModel>(
                builder: (context, wrapperModel, child) {
                  return Text(
                    'LEVEL ${pow(2 * ee.Xp!, (1 / 3)).floor()}  ${ee.name} (${ee.Xp!} XP)',
              ...
              TextButton(
                ...
                //       content: ee,
                //       context: context,
                //       instance: instance as Weaponsxpwindow,
                //     ).build(context),
                onPressed: () {
                  xpOverlay(content: ee, ctx: context).build(context);
                  // Provider.of<WrapperModel>(context, listen: false).addXp(ee, 10);
                },
                child: Text("Add XP"),
              ),
            ],
  ...
  Widget build(BuildContext context) {
    return Column(children: makeXPSpinboxes(context));
  }
}
